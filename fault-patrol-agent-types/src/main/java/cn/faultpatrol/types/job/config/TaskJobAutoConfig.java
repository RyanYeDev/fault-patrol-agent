package cn.faultpatrol.types.job.config;

import cn.faultpatrol.types.job.TaskJob;
import cn.faultpatrol.types.job.provider.ITaskDataProvider;
import cn.faultpatrol.types.job.service.ITaskJobService;
import cn.faultpatrol.types.job.service.TaskJobService;
...
@ConditionalOnProperty(prefix = "faultpatrol.task.job", name = "enabled", havingValue = "true", matchIfMissing = true)
public class TaskJobAutoConfig {
    ...
    public TaskScheduler taskScheduler(TaskJobAutoProperties properties) {
        ...
        log.info("fault-patrol，任务调度器初始化完成。线程池大小: {}, 线程名前缀: {}", 
    ...
    public TaskJob taskJob(TaskJobAutoProperties properties, ITaskJobService taskJobService) {
        log.info("fault-patrol，任务调度作业初始化完成。刷新间隔: {}ms, 清理cron: {}", 
