package cn.faultpatrol.infrastructure.adapter.repository;

import cn.faultpatrol.domain.agent.adapter.repository.IAgentRepository;
import cn.faultpatrol.domain.agent.model.valobj.*;
import cn.faultpatrol.infrastructure.dao.*;
import cn.faultpatrol.infrastructure.dao.po.*;
...
import static cn.faultpatrol.domain.agent.model.valobj.enums.AiAgentEnumVO.*;
...
public class AgentRepository implements IAgentRepository {
    ...
    @Override
    public void saveDiagnosisReport(DiagnosisReportVO diagnosisReportVO) {
        DiagnosisReport diagnosisReport = DiagnosisReport.builder()
                .sessionId(diagnosisReportVO.getSessionId())
                .agentId(diagnosisReportVO.getAgentId())
                .alertContent(diagnosisReportVO.getAlertContent())
                ...
                .build();
        diagnosisReportDao.insert(diagnosisReport);
    }

    @Override
    public List<DiagnosisReportVO> queryDiagnosisReportsBySessionId(String sessionId) {
        List<DiagnosisReport> reports = diagnosisReportDao.queryBySessionId(sessionId);
        return reports.stream().map(this::toReportVO).collect(Collectors.toList());
    }

    @Override
    public DiagnosisReportVO queryDiagnosisReportById(Long id) {
        DiagnosisReport report = diagnosisReportDao.queryById(id);
        if (report == null) {
            return null;
        }
    ...
    private DiagnosisReportVO toReportVO(DiagnosisReport report) {
        return DiagnosisReportVO.builder()
                ...
                .alertContent(report.getAlertContent())
